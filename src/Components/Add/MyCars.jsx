    ...
    fetch("https://rentcar-seven.vercel.app/allcars")
      ...
      .then((data) => {
        ...
        const filteredData = data.filter(
          (car) => car.userDetails.email === user.email
        );
        setMyData(filteredData);
        setDataloading(false);
      })
  ...
  const handleDelete = (data) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://rentcar-seven.vercel.app/car/${data._id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then(() => {
    ...
    <div className="dark:text-white  dark:bg-black">
      <div className="min-h-screen px-[10vw] text-black p-8 pt-24 dark:text-white  dark:bg-black">
        ...
        {myData.map((data) => (
          <div
            key={data._id}
            className="mx-auto border-t p-5 gap-9 pb-9 border-white flex flex-col xl:flex-row items-start space-y-10 dark:border-white"
          >
